...
async function testRazorpaySubscription() {
  ...
  try {
    ...
    const orderResponse = await fetch('http://localhost:5000/api/create-subscription-order', {
      method: 'POST',
      headers: {
    ...
    console.log('\n2. Verifying payment...');
    const verifyResponse = await fetch('http://localhost:5000/api/verify-subscription-payment', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'x-dev-user-email': 'test@example.com', // Use development mode auth
    ...
    const subscriptionResponse = await fetch('http://localhost:5000/api/user/subscription/test-user', {
      method: 'GET',
      headers: {
        'x-dev-user-email': 'test@example.com', // Use development mode auth
      }
    ...
    if (subscriptionResponse.status !== 404 && !subscriptionResponse.ok) {
      throw new Error(`Failed to check subscription: ${subscriptionResponse.statusText}`);
    }
