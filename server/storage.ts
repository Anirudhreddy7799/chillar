import {
  ...
  Draw, InsertDraw,
  Claim, InsertClaim,
  users, subscriptions, rewards, draws, claims
} from "@shared/schema";
import { db } from "./db";
import { and, eq, gte, lt, desc, asc, or } from "drizzle-orm";
...
export interface IStorage {
  ...
  getSubscriptionByRazorpayId(razorpaySubId: string): Promise<Subscription | undefined>;
  ...
  getDrawsWithWinners(): Promise<(Draw & { winner?: User, reward?: Reward })[]>;

  // Claim operations
  getClaim(id: number): Promise<Claim | undefined>;
  getClaimByUserAndReward(userId: number, rewardId: number): Promise<Claim | undefined>;
...
export class MemStorage implements IStorage {
  ...
  async getSubscriptionByRazorpayId(razorpaySubId: string): Promise<Subscription | undefined> {
    return Array.from(this.subscriptions.values()).find(sub => sub.razorpaySubId === razorpaySubId);
  }
  ...
  async getRewardByWeek(week: string): Promise<Reward | undefined> {
    return Array.from(this.rewards.values()).find(reward => reward.week === week);
  }
  ...
  async getDrawsWithWinners(): Promise<(Draw & { winner?: User; reward?: Reward })[]> {
    ...
    return await Promise.all(draws.map(async draw => {
      const winner = draw.winnerId ? await this.getUser(draw.winnerId) : undefined;
      const reward = draw.rewardId ? await this.getReward(draw.rewardId) : undefined;
      return { ...draw, winner, reward };
    }));
  }
  ...
  async getPendingClaims(): Promise<(Claim & { user?: User; reward?: Reward })[]> {
    ...
    return await Promise.all(pendingClaims.map(async claim => {
  ...
  async getUserClaims(userId: number): Promise<(Claim & { reward?: Reward })[]> {
    ...
    return await Promise.all(userClaims.map(async claim => {
      const reward = await this.getReward(claim.rewardId);
      return { ...claim, reward };
    }));
  }
...
export class DatabaseStorage implements IStorage {
  ...
  async getRewardByWeek(week: string): Promise<Reward | undefined> {
    const [reward] = await db.select().from(rewards).where(eq(rewards.week, week));
    return reward;
  }
  ...
  async getUpcomingRewards(): Promise<Reward[]> {
    ...
    const result = await db.select().from(rewards).orderBy(rewards.week);
  ...
  async getDrawsWithWinners(): Promise<(Draw & { winner?: User; reward?: Reward })[]> {
    const drawsData = await db.select().from(draws);
    ...
    return await Promise.all(
      drawsData.map(async (draw) => {
        ...
        if (draw.winnerId) {
          [winner] = await db.select().from(users).where(eq(users.id, draw.winnerId));
        }
        
        if (draw.rewardId) {
          [reward] = await db.select().from(rewards).where(eq(rewards.id, draw.rewardId));
        }
...
// Import FirebaseStorage class
import { FirebaseStorage } from './firebase-storage';

// This flag can be used to switch between database storage and Firestore storage
const USE_FIREBASE_AS_PRIMARY = true;

// Export the appropriate storage implementation
export const storage = USE_FIREBASE_AS_PRIMARY 
  ? new FirebaseStorage() 
  : new DatabaseStorage();
