...
import { updateUserData, updateUserEmail, updateUserProfile, logout, getUserData } from '@/firebase';
...
const Dashboard = () => {
  ...
  useEffect(() => {
    const fetchCurrentReward = async () => {
      try {
        const response = await apiRequest('GET', '/api/rewards/current', undefined);
      ...
      try {
        ...
        const convertFirebaseTimestamp = (timestamp: any): Date | null => {
          ...
          try {
            ...
            if (typeof timestamp === 'object' && timestamp.seconds) {
              ...
              const parsedDate = new Date(seconds * 1000 + nanoseconds / 1000000);
              return isNaN(parsedDate.getTime()) ? null : parsedDate;
            }
          } catch (error) {
        ...
        const fetchFromFirebase = async () => {
          try {
            ...
            console.log('Fetched data from Firebase:', firebaseData ? 'success' : 'no data');
            return firebaseData;
          } catch (error) {
        ...
        if (apiData && firebaseData && firebaseData.profile) {
          ...
          const firebaseBirthday = firebaseData.profile.birthday ? 
            convertFirebaseTimestamp(firebaseData.profile.birthday) : null;
          ...
          const mergedData = {
            ...
            avatarUrl: firebaseData.profile.avatarUrl || apiData.avatarUrl || '',
            referralCode: firebaseData.referralCode || apiData.referralCode || '',
  ...
  useEffect(() => {
    if (userProfile) {
      ...
      if (!userProfile.name && !userProfile.birthday && 
          ...
          !userProfile.avatarUrl && !userProfile.referralCode) {
      ...
      if (userProfile.birthday) {
        try {
          ...
          } else if (typeof userProfile.birthday === 'object' && userProfile.birthday.seconds) {
          ...
          if (birthdayDate && !isNaN(birthdayDate.getTime())) {
            console.log("Successfully parsed birthday:", birthdayDate.toISOString());
          } else {
  ...
  const updateProfileMutation = useMutation({
    mutationFn: async (profileData: UserProfile) => {
      ...
      if (!profileData.phone?.trim() || !/^[6-9]\d{9}$/.test(profileData.phone)) {
        throw new Error('Valid phone number is required (10 digits starting with 6-9)');
      }
      ...
      try {
        ...
        try {
          const response = await apiRequest('PATCH', `/api/user/profile/${currentUser.uid}`, profileToUpdate);
    ...
    <Layout>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="mb-6">
          ...
          <p className="text-muted-foreground">Welcome back, <span>{displayUserData.email}</span>!</p>
        </div>
                ...
                <Card>
                    ...
                    <div className="grid grid-cols-1 gap-4">
                      ...
                      <div className="space-y-2">
                        ...
                        <Input 
                          ...
                          className={!isEditing ? "cursor-pointer bg-muted" : ""}
                          readOnly={!isEditing}
                      ...
                      <div className="space-y-2">
                        ...
                        <div className="py-2 px-3 border rounded-md bg-muted">{userProfile?.referralCode || 'Loading...'}</div>
                        ...
                        {isEditing ? (
                          <div className="grid grid-cols-3 gap-2">
                            <div>
                              ...
                              <Select
                                value={profileData.birthday ? new Date(profileData.birthday).getDate().toString() : ''}
                                onValueChange={(value) => {
                                  const day = parseInt(value);
                                  if (isNaN(day) || day < 1 || day > 31) return;
                                  
                                  const currentDate = profileData.birthday ? new Date(profileData.birthday) : new Date();
                                  currentDate.setDate(day);
                                  setProfileData({...profileData, birthday: currentDate});
                                }}
                              >
                            ...
                            <div>
                              ...
                              <Select
                                value={profileData.birthday ? (new Date(profileData.birthday).getMonth() + 1).toString() : ''}
                                onValueChange={(value) => {
                            ...
                            <div>
                              ...
                              <Select
                                value={profileData.birthday ? new Date(profileData.birthday).getFullYear().toString() : ''}
                                onValueChange={(value) => {
                                  const year = parseInt(value);
                                  if (isNaN(year) || year < 1940 || year > new Date().getFullYear() - 13) return;
                          ...
                          >
                            {profileData.birthday ? format(profileData.birthday, "PPP") : 'Click to set your birthday (Required)'}
                          </div>
                        )}
                      ...
                      <div className="space-y-2">
                        ...
                        <p className="text-xs text-muted-foreground">Select your city or district</p>
                      </div>
                      ...
                      <div className="space-y-2">
                        ...
                        <Input 
                          ...
                          pattern="[6-9][0-9]{9}"
                        />
                        <p className="text-xs text-muted-foreground">Indian mobile number (10 digits starting with 6-9)</p>
                      </div>
                    </div>
                  </CardContent>
                  <CardFooter className="flex justify-between">
                    {isEditing ? (
                        ...
                        <Button 
                          ...
                          disabled={
                            ...
                            (userProfile?.profile && 
                              ...
                              (userProfile.profile.birthday?.seconds === profileData.birthday?.getTime() / 1000 || 
                                (userProfile.profile.birthday && profileData.birthday && 
                                 new Date(userProfile.profile.birthday.seconds * 1000).getTime() === profileData.birthday.getTime())) &&
                              userProfile.profile.location === profileData.location
                            )
                          }
                        >
                      ...
                      <Button onClick={() => setIsEditing(true)}>Edit All Fields</Button>
                    )}
                  </CardFooter>
                </Card>
              </TabsContent>
                ...
                <div className="mt-6">
                  {rewardLoading ? (
                    ...
                    <CurrentReward
                      ...
                      description={displayReward.description || `${displayReward.prizeType === 'voucher' 
                    ...
                    <Card className="bg-card rounded-xl shadow-lg">
                      <CardContent className="p-6 text-center">
                        <p className="text-xl font-semibold text-white mb-4">No active reward this week</p>
                        <p className="text-muted-foreground">Check back soon for upcoming rewards!</p>
                      </CardContent>
                    </Card>
                  )}
                </div>
