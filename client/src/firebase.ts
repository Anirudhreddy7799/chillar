...
if (
  ...
  firebaseConfig.apiKey === "FIREBASE_API_KEY_PLACEHOLDER"
) {
  console.error(
    "Firebase API Key is missing or invalid! Please set VITE_FIREBASE_API_KEY environment variable."
  );
}
if (
  ...
  console.error(
    "Firebase Project ID is missing or invalid! Please set VITE_FIREBASE_PROJECT_ID environment variable."
  );
}
if (
  !firebaseConfig.appId ||
  firebaseConfig.appId === "FIREBASE_APP_ID_PLACEHOLDER"
) {
  console.error(
    "Firebase App ID is missing or invalid! Please set VITE_FIREBASE_APP_ID environment variable."
  );
}
...
try {
  // Validate the Firebase configuration before initializing
  validateFirebaseConfig();
  ...
  try {
    ...
    console.error(
      "Error initializing Firestore (continuing without it):",
      firestoreError
    );
  }
} catch (error) {
...
export const register = async (
  ...
  try {
    if (!auth) {
      console.error(
        "Firebase auth is not initialized, likely due to missing or invalid Firebase configuration"
      );
      throw new Error(
    ...
    if (db) {
      try {
        ...
        console.error(
          "Error saving to Firestore (continuing):",
          firestoreError
        );
      }
    } else {
      console.warn(
        "Firestore not initialized, skipping Firestore data storage"
      );
    }
    ...
    if (error.code === "auth/configuration-not-found") {
      console.error(
        "Firebase configuration error: The Firebase project may not be properly set up or the domain may not be authorized."
      );
  ...
  try {
    ...
    try {
      const response = await fetch(
        `/api/user/profile/${userCredential.user.uid}`
      );
    ...
    if (error.code === "permission-denied") {
      ...
      try {
        ...
        if (response.ok) {
          const userData = await response.json();
          console.log(
            "Retrieved user from PostgreSQL (Firestore permission denied)"
          );
          return userData;
        }
      } catch (dbError) {
  ...
  try {
    ...
    if (error.code === "permission-denied") {
      ...
      try {
        ...
        if (response.ok) {
          console.log(
            "User data updated in PostgreSQL database (Firestore permission denied)"
          );
          return true;
        }
      } catch (dbError) {
    ...
    try {
      ...
      if (userDoc.exists()) {
        ...
        if (userData.id) {
          ...
          const response = await fetch(
            `/api/user/email/update/${auth.currentUser.uid}`,
            {
              method: "PATCH",
              headers: {
                "Content-Type": "application/json",
          ...
          if (response.ok) {
            ...
            console.warn(
              "Failed to sync email to PostgreSQL database using UID"
            );
          }
        }
      }
      ...
      if (userDoc.exists()) {
        ...
        if (userData.id) {
          ...
          if (updateResponse.ok) {
            ...
            console.warn(
              "Failed to sync profile to PostgreSQL database using UID"
            );
          }
        }
      }
...
export const getUserByReferralCode = async (referralCode: string) => {
  try {
    ...
    if (error.code === "permission-denied") {
      console.error(
        "Error fetching user by referral code from Firestore:",
        error
      );
      console.warn(`
